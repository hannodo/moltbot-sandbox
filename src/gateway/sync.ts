...
export async function syncToR2(sandbox: Sandbox, env: MoltbotEnv): Promise<SyncResult> {
  ...
  // Run rsync to backup state to R2
  // Note: Use --no-times because s3fs doesn't support setting timestamps.
  // OpenClaw OAuth profiles are stored under /root/.openclaw and must be persisted.
  const syncCmd = `rsync -r --no-times --delete --exclude='*.lock' --exclude='*.log' --exclude='*.tmp' /root/.clawdbot/ ${R2_MOUNT_PATH}/clawdbot/ && rsync -r --no-times --delete /root/clawd/skills/ ${R2_MOUNT_PATH}/skills/ && (test ! -d /root/.openclaw || rsync -r --no-times --delete --exclude='*.lock' --exclude='*.log' --exclude='*.tmp' /root/.openclaw/ ${R2_MOUNT_PATH}/openclaw-home/) && date -Iseconds > ${R2_MOUNT_PATH}/.last-sync`;
  ...
  try {
    ...
    // Note: backup structure is ${R2_MOUNT_PATH}/clawdbot/, ${R2_MOUNT_PATH}/skills/, and ${R2_MOUNT_PATH}/openclaw-home/
